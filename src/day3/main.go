...
func calcManhattanDist(p point) int {
...
func findIntersectionDist(wire1Paths []string, wire2Paths []string) (int, error) {
	var closestDist int
	wire1Points := make(map[point]bool)
	currPoint := point{x: 0, y: 0}

	for _, path := range wire1Paths {
		direction := path[:1]
		...
		default:
			return 0, fmt.Errorf("Received invalid direction: %v", direction)
		}
	}

	currPoint = point{x: 0, y: 0}
	for _, path := range wire2Paths {
		direction := path[:1]
		distance, err := strconv.Atoi(path[1:])
		if err != nil {
			return 0, err
		...
		case "R":
			for i := 1; i <= distance; i++ {
				currPoint.x++
				_, ok := wire1Points[currPoint]
				if ok {
					fmt.Printf("intersected at: %v\n", currPoint)
					intersectionDist := calcManhattanDist(currPoint)
					if closestDist == 0 {
						closestDist = intersectionDist
					} else {
						closestDist = min(closestDist, intersectionDist)
					}
				}
			}
		case "L":
...
func findIntersectionCost(wire1Paths []string, wire2Paths []string) (int, error) {
	var cost, lowestCost int
	...
	for _, path := range wire2Paths {
		...
		case "R":
			for i := 1; i <= distance; i++ {
				cost++
				currPoint.x++
				wire1Cost, ok := wire1Points[currPoint]
				if ok {
					fmt.Printf("wire1 cost: %v, wire2 cost: %v at point %v\n", wire1Cost, cost, currPoint)
					intersectionCost := wire1Cost + cost
					if lowestCost == 0 {
						lowestCost = intersectionCost
					} else {
						lowestCost = min(lowestCost, intersectionCost)
					}
				}
			}
		case "L":
...
func main() {
	...
	// puzzle 2 answer
	cost, err := findIntersectionCost(strings.Split(puzzleInput.wire1Path, ","), strings.Split(puzzleInput.wire2Path, ","))
	if err != nil {
		fmt.Printf("Encountered error: %v", err)
	}
